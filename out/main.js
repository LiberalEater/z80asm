...
class main {
       ...
       constructor(symbolDocumenter, diagnosticProvider) {
              ...
              const scanDoc = async (document, event) => {
                     if (!document.fileName.match(/\.(z80|inc|asm)$/i)) {
                            return
                     }
              ...
              vscode.workspace.findFiles("**/*{Main,main}.{z80,asm}", null, 1).then((files) => {
              ...
              vscode.workspace.findFiles("**/*.{z80,asm}", null, 2).then((files) => {
                     files.forEach((fileURI) => {
                            vscode.workspace.openTextDocument(fileURI).then((document) => {
                                   if (!this.symbolDocumenter.documents[document.uri.fsPath]) {
                                          scanDoc(document);
              ...
              const docOpened = (document) => { // if a file was just opened
                     if (!this.symbolDocumenter.documents[document.uri.fsPath] && document.fileName.match(/\.(z80|inc|asm)$/i)) {
                            scanDoc(document)
